...
uint8_t SWITCH_PIN = D0;       // declare LED pin on NodeMCU Dev Kit
...
bool onPowerState(const String &deviceId, bool &state) {
  ...
  myPowerState = state;
  digitalWrite(SWITCH_PIN, myPowerState?HIGH:LOW);
...
void handleButtonPress() {
  ...
  if (digitalRead(BTN_FLASH) == LOW && actualMillis - lastBtnPress > 1000)  { // is button pressed (inverted logic! button pressed = LOW) and debounced?
    ...
    digitalWrite(SWITCH_PIN, myPowerState?LOW:HIGH); // if myPowerState indicates device turned on: turn on led (builtin led uses inverted logic: LOW = LED ON / HIGH = LED OFF)
...
void setup() {
  ...
  digitalWrite(SWITCH_PIN, HIGH); // turn off LED on bootup
